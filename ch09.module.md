...
| `-file(File,Line)` | 修改了预定义的宏 `?File` 和 `?Line` 的值. | `__ENV__.file`, `__ENV__.line`, `@file`, `@line` |
...
[^attr]: 持久化属性值在编程中的使用, 见后面的[冷知识章节](./ch.clod_knowdage.md).
...
有关协议与行为的更多讨论, 见后面的
[&Lt;协议与行为&Gt;](./ch10.protocol_and_behaviour.md) 章节.
