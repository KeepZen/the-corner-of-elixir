...
就可以了. 然后运行 `mix get deps` 就会下载对应的代码到本地.
...
为实现代码复用, Elixir 提供了几个特殊表达: `import`, `require` 和 `use`.
...
```elixir
require <Module_name> [,as: <Alias_Name>]
``` 
...
我们可以为这个定义一个新的宏来改变 `require` 对 `:as` 选项的默认处理,
使得它可以像 `aliase` 那样来处理 `:as` 选项.
...
* 导入模块中多有的公开的宏和函数: `import ModuleName`
...
不推荐使用第 1 中格式, 因为这样的代码, 在代码中, 难以从模块中到底导入了什么函数.
而应该优先使用第 2 中格式.

第 3 中格式一般用来覆盖核心模块提供的函数或宏的时候使用.
...
为了减少冗余, 可以在 `OverWriteAdd` 模块中添加宏或函数 `__using__`.
