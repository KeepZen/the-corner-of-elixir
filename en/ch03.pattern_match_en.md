...
```elixir
@exclude_atoms [:^, :%{}, :%, :{}]
defp prewalker({atom, meta, args}) when atom in @exclude_atoms do
  args =
    Enum.map(
      args,
      fn
        ...
        {atom, other} when atom not in @exclude_atoms ->
...
在 Erlang OTP 25 中引入了一个新特性 maybe, 其中可以使用新的短路操作符 `?=`[^1],
...
见本书的第5章 ≪[定制新结构](ch05.new_constructor_en.md)≫.
...
更多的内容见后面的[错误处理章节](./ch08.error_handle_en.md).

[^1]: 见 [≪Erlang 参考手册•maybe≫ 章节](https://www.erlang.org/doc/reference_manual/expressions.html#maybe)