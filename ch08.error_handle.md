...
按照 [Elixir 文档](https://hexdocs.pm/elixir/Kernel.SpecialForms.html#try/1)
...
在 Elixir 中 `catch` 并没有退化. 而Elixir 中增加的保留字 `resuce`,
...
[^rescue]: Elixir 文档中的话, 翻译过来是: 除了以模式匹配外,
...
但是, 注意对比运行时产生的匹配错误, 和使用 `raise` 产生的匹配错误之间的微妙区别.
...
也就是说, 在 `try` 子句中: 如果 `rescue` 和 `catch` 都存在的话,
`rescue` 应该放在 `catch` 子句的前面,
而其他的子句的顺序并没有特别的要求.
...
[&Lt;保留字&Gt;](https://hexdocs.pm/elixir/syntax-reference.html#reserved-words)
小节也给出了提示:
